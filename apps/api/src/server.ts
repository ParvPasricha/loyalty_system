...
async function getPointsBalance(merchantId: string, customerId: string): Promise<number> {
  const result = await query<{ balance: string }>(
    "SELECT COALESCE(SUM(points_delta), 0)::text AS balance FROM ledger_entries WHERE merchant_id = $1 AND customer_id = $2",
...
app.post("/public/session/init", { config: { rateLimit: { max: 30, timeWindow: "1 minute" } } }, async (request, reply) => {
  ...
  const existingDevice = await query<{ customer_id: string }>(
    "SELECT customer_id FROM customer_devices WHERE merchant_id = $1 AND device_id = $2",
    [merchantId, body.device_id],
  );
  if (existingDevice.rows[0]) {
    const token = await query<{ public_token: string }>(
      ...
      [merchantId, existingDevice.rows[0].customer_id],
    );
  ...
  await withTransaction(async (client) => {
    ...
    await client.query(
      ...
      [merchantId, customerId, body.device_id],
    );
  });
...
app.get("/public/card/:publicToken", { config: { rateLimit: { max: 60, timeWindow: "1 minute" } } }, async (request, reply) => {
  ...
  const tokenResult = await query<{ merchant_id: string; customer_id: string }>(
    "SELECT merchant_id, customer_id FROM customer_tokens WHERE public_token = $1 AND status = 'active'",
    [params.publicToken],
  );
  ...
  const merchantResult = await query<{ name: string; slug: string }>(
    "SELECT name, slug FROM merchants WHERE id = $1",
    [tokenRow.merchant_id],
  );
  const rewards = await query<{ id: string; name: string; points_cost: number }>(
    "SELECT id, name, points_cost FROM rewards WHERE merchant_id = $1 AND active = true",
    [tokenRow.merchant_id],
  );
...
app.post("/tokens/resolve", { preHandler: requireRole(["cashier", "manager", "owner"]) }, async (request, reply) => {
  ...
  const tokenResult = await query<{ merchant_id: string; customer_id: string; id: string }>(
    "SELECT id, merchant_id, customer_id FROM customer_tokens WHERE public_token = $1 AND status = 'active'",
    [body.public_token],
  );
  const tokenRow = tokenResult.rows[0];
  if (!tokenRow) {
...
app.post("/tokens/revoke", { preHandler: requireRole(["manager", "owner"]) }, async (request, reply) => {
  ...
  const tokenResult = await query<{ id: string }>(
    "UPDATE customer_tokens SET status = 'revoked', revoked_at = now() WHERE (id = $1 OR public_token = $2) RETURNING id",
    [body.token_id ?? null, body.public_token ?? null],
  );
...
app.post("/tokens/bind-nfc", { preHandler: requireRole(["manager", "owner"]) }, async (request, reply) => {
  ...
  await query(
    "UPDATE customer_tokens SET metadata = jsonb_set(metadata, '{nfc_uid}', to_jsonb($1::text), true) WHERE public_token = $2",
    [body.nfc_uid, body.public_token],
  );
  return reply.send({ success: true });
});
...
app.post("/earn", { preHandler: requireRole(["cashier", "manager", "owner"]) }, async (request, reply) => {
  ...
  const rulesResult = await query<{ id: string; points_per_dollar: string; rounding: "floor" | "nearest"; promo_multiplier: string }>(
    "SELECT id, points_per_dollar, rounding, promo_multiplier FROM loyalty_rules_versions WHERE merchant_id = $1 AND active_from <= now() ORDER BY version DESC LIMIT 1",
    [tokenRow.merchant_id],
  );
  ...
  await withTransaction(async (client) => {
    const insertResult = await client.query(
      "INSERT INTO ledger_entries (merchant_id, customer_id, type, points_delta, source, rules_version_id, idempotency_key) VALUES ($1, $2, 'earn', $3, 'terminal', $4, $5) ON CONFLICT (merchant_id, idempotency_key) DO NOTHING",
      [tokenRow.merchant_id, tokenRow.customer_id, points, rules.id, body.idempotency_key],
    );
...
app.post("/redeem", { preHandler: requireRole(["cashier", "manager", "owner"]) }, async (request, reply) => {
  ...
  await withTransaction(async (client) => {
    const existing = await client.query<{ id: string }>(
      "SELECT id FROM ledger_entries WHERE merchant_id = $1 AND idempotency_key = $2",
      [tokenRow.merchant_id, body.idempotency_key],
    );
    if (existing.rows[0]) {
      return;
    }
    await client.query("SELECT id FROM customers WHERE id = $1 FOR UPDATE", [tokenRow.customer_id]);
    const rewardResult = await client.query<{ id: string; points_cost: number }>(
      "SELECT id, points_cost FROM rewards WHERE id = $1 AND merchant_id = $2 AND active = true",
      [body.reward_id, tokenRow.merchant_id],
    );
    ...
    const balanceResult = await client.query<{ balance: string }>(
      "SELECT COALESCE(SUM(points_delta), 0)::text AS balance FROM ledger_entries WHERE merchant_id = $1 AND customer_id = $2",
      [tokenRow.merchant_id, tokenRow.customer_id],
    );
    ...
    await client.query(
      "INSERT INTO ledger_entries (merchant_id, customer_id, type, points_delta, source, idempotency_key) VALUES ($1, $2, 'redeem', $3, 'terminal', $4)",
      [tokenRow.merchant_id, tokenRow.customer_id, -reward.points_cost, body.idempotency_key],
    );
    await client.query(
      "INSERT INTO redemptions (merchant_id, customer_id, reward_id, points_cost) VALUES ($1, $2, $3, $4)",
      [tokenRow.merchant_id, tokenRow.customer_id, reward.id, reward.points_cost],
    );
    await client.query(
      "INSERT INTO audit_logs (merchant_id, staff_user_id, action, target_type, target_id, meta) VALUES ($1, $2, 'redeem', 'reward', $3, $4)",
      [tokenRow.merchant_id, request.staff?.staff_user_id ?? null, reward.id, { idempotency_key: body.idempotency_key }],
    );
  });
...
app.get("/customers/:id/ledger", { preHandler: requireRole(["cashier", "manager", "owner"]) }, async (request, reply) => {
  ...
  const entries = await query(
    "SELECT id, type, points_delta, source, external_id, created_at FROM ledger_entries WHERE merchant_id = $1 AND customer_id = $2 ORDER BY created_at DESC",
...
app.get("/merchant/settings", { preHandler: requireRole(["manager", "owner"]) }, async (request, reply) => {
  ...
  const merchant = await query(
    "SELECT id, slug, name FROM merchants WHERE id = $1",
    [merchantId],
  );
  const rules = await query(
    "SELECT version, points_per_dollar, rounding, promo_multiplier, active_from FROM loyalty_rules_versions WHERE merchant_id = $1 ORDER BY version DESC LIMIT 1",
    [merchantId],
  );
...
app.post("/merchant/settings", { preHandler: requireRole(["manager", "owner"]) }, async (request, reply) => {
  ...
  const latest = await query<{ version: number }>(
    "SELECT COALESCE(MAX(version), 0)::int AS version FROM loyalty_rules_versions WHERE merchant_id = $1",
    [merchantId],
  );
  const nextVersion = (latest.rows[0]?.version ?? 0) + 1;
  await query(
    "INSERT INTO loyalty_rules_versions (merchant_id, version, points_per_dollar, rounding, promo_multiplier, active_from) VALUES ($1, $2, $3, $4, $5, now())",
    [merchantId, nextVersion, body.points_per_dollar, body.rounding, body.promo_multiplier],
  );
...
app.get("/merchant/rewards", { preHandler: requireRole(["manager", "owner"]) }, async (request, reply) => {
  ...
  const rewards = await query(
    "SELECT id, name, points_cost, active FROM rewards WHERE merchant_id = $1 ORDER BY created_at DESC",
    [merchantId],
  );
...
app.get("/merchant/qr", { preHandler: requireRole(["manager", "owner"]) }, async (request, reply) => {
  const merchantId = request.staff?.merchant_id;
  const merchant = await query<{ slug: string }>(
